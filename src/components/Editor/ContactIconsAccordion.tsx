            ...
            <AccordionItem value="item-1">
                ...
                <AccordionContent className="border rounded-md border-t-transparent p-1 flex flex-row flex-wrap gap-1 justify-start">  
                    {SocialLists.map(({type, icon}) => {
                        const initialInputVal = socials.find(v => v.socialType === type)?.socialValue ?? ''

                        const bgColor = initialInputVal ? "bg-blue-200 hover:bg-blue-300" : "bg-gray-100 hover:bg-gray-200"
                        return (
                            <Dialog key={type}>
                            <DialogTrigger key={type} className={`${bgColor} text-[#7b7b7b] text-xl p-2 rounded-sm  transition-all duration-150`}>
                                {icon}
                            </DialogTrigger>
                            <DialogContent>
                              <DialogHeader>
